...
def split(X, y, val_size, test_size=None, max_size=None, random_state=1):
    ...
    idx_test = idx[:test_size]#test set
    idx_val = idx[test_size: test_size + val_size]#validation set
...
def inject_mv(X_train, y_train, mv_prob, mv_type, feature_importance):
    ...
    # inject missing values，另外用一个矩阵来表示缺失单元位置
    mask = np.random.rand(*X_train_mv.shape) <= m_prob_matrix

    # avoid injecting in all columns for one row 确保均匀
...
def build_synthetic_dataset(data_dir, dataset, val_size=1000, test_size=None,
    ...
    # select important features（使用深度学习模型做不到）？？？？？
    #但或许 我们可以用KNN获取
    ...
    if save_dir is not None:#把data_dict都保存起来
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mv_prob', default=0.3, type=float)
