...
def repair(X_train_mv, save_dir=None):#针对缺失的训练集进行修补
    ...
    if is_num and is_cat:#数字和类别都缺失，使用混合插补
        all_imputers = mix_imputers
    elif is_num:#缺失数字，使用数字插补
        all_imputers = num_imputers
    elif is_cat:#缺失种类，使用类别插补
        ...
        for i in range(len(domain) - 1):#全部不含缺失值的，不含缺失值的唯一值
